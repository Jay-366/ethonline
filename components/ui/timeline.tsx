      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-40 pt-0">
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <div className="hidden md:block md:pl-20">
                <h3 className="text-xl md:text-5xl font-bold mb-2" style={{ color: '#5d606c' }}>
                  {item.title}
                </h3>
                {item.subtitle && (
                  <h4 className="text-lg md:text-2xl font-bold mb-1" style={{ color: '#f8ede0' }}>
                    {item.subtitle}
                  </h4>
                )}
                {item.description && (
                  <p className="text-sm md:text-base" style={{ color: '#5d606c' }}>
                    {item.description}
                  </p>
                )}
              </div>
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <div className="md:hidden block mb-4">
                <h3 className="text-2xl mb-2 text-left font-bold" style={{ color: '#5d606c' }}>
                  {item.title}
                </h3>
                {item.subtitle && (
                  <h4 className="text-xl font-bold mb-1" style={{ color: '#f8ede0' }}>
                    {item.subtitle}
                  </h4>
                )}
                {item.description && (
        ...
        >
          <div 
            ...
            style={{ 
              background: 'linear-gradient(to bottom, transparent 0%, #5d606c 10%, #5d606c 90%, transparent 100%)'
            }}
          />
          <motion.div
            style={{
              ...
              background: 'linear-gradient(to top, #f8ede0 0%, #f8ede0 10%, transparent 100%)'
            }}
