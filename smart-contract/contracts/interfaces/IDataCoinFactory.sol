...
interface IDataCoinFactory {
    ...
    function createDataCoin(
        ...
        uint256 lockAmount,
        bytes32 salt
    ) external returns (address coinAddress, address poolAddress);
    ...
    function canWithdrawLPTokens(
        address coinAddress
    ) external view returns (bool canWithdraw, uint256 timeRemaining);

    function dataCoinCreationFeeBPS() external view returns (uint256);

    function dataCoinInfo(
        address coinAddress
    ) external view returns (DataCoinInfo memory);
