...
/**
 * @title DataCoinVault
 * @dev A secure vault for managing ERC20 tokens with owner-controlled withdrawals
 * @notice This contract allows the owner to safely store and withdraw various ERC20 tokens
 */
contract DataCoinVault is Ownable, ReentrancyGuard {
    ...
    function withdrawTokens(
        ...
        require(tokenAddress != address(0), "DataCoinVault: Token address cannot be zero");
        require(recipient != address(0), "DataCoinVault: Recipient address cannot be zero");
        require(amount > 0, "DataCoinVault: Amount must be greater than zero");
        
        IERC20 token = IERC20(tokenAddress);
        uint256 vaultBalance = token.balanceOf(address(this));
        
        require(vaultBalance >= amount, "DataCoinVault: Insufficient vault balance");
        
        // Transfer tokens to recipient
        bool success = token.transfer(recipient, amount);
        require(success, "DataCoinVault: Token transfer failed");
    ...
    function emergencyWithdrawAll(
        ...
        require(entireBalance > 0, "DataCoinVault: No tokens to withdraw");
        
        bool success = token.transfer(recipient, entireBalance);
        require(success, "DataCoinVault: Emergency withdrawal failed");
     ...
     */
    function trackManualDeposit(address tokenAddress) external onlyOwner {
        require(tokenAddress != address(0), "DataCoinVault: Token address cannot be zero");
